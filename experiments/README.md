...
A fixed-game run is identified by the game and payoff digest, feedback, ordered learner profile, horizon, base seed, replicate, stationary solver, implementation version, and runtime-environment fingerprint. Incompatible historical CSVs are excluded from current analysis without migration or rewriting. Player `i` in replicate `r` of a `p`-player game receives seed:
