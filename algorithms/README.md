...
| `AuerExp3` | Auer et al. (2002), Figure 1: `x_hat[k]=r_k/p_k`; probabilities explicitly mix normalized weights with uniform exploration | [Blum–Mansour (2007), Theorem 11](https://www.jmlr.org/papers/volume8/blum07a/blum07a.pdf): `gamma=min(1,sqrt(K log(K)/T))`, `eta=gamma/K`, and `w_k <- w_k exp(eta x_hat[k])` | Canonical Bandit BM inner learner; the pathwise bound used in Lemma 10 yields swap pseudo-regret `O(K sqrt(K T log K))` for the reduction with `B_max=T` |
...
`AuerExp3` uses the algorithmic update from [Auer et al. (2002), Figure 1](https://www.schapire.net/papers/AuerCeFrSc01.pdf) with the parameter choice in the proof of Blum–Mansour Theorem 11. Its `B_max=T` is valid because `B_{i,j}=E[sum_t p_i^t b_j^t] <= T` for rewards in `[0,1]`. The absence of an `e-1` factor is intentional: that factor belongs to Auer's standalone Corollary 3.2 tuning, which is not the parameter choice used here.
